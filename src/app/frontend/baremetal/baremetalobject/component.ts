...
import {BaremetalObjectDetail} from '@api/backendapi';
...
@Component({selector: 'kd-bmh-object-detail', templateUrl: './template.html'})
export class BaremetalObjectDetailComponent implements OnInit, OnDestroy {
  ...
  private readonly endpoint_ = EndpointManager.resource(Resource.baremetalhost, true);
  object: BaremetalObjectDetail;
  modes = Modes;
  isInitialized = false;
  ...
  constructor(
    private readonly object_: NamespacedResourceService<BaremetalObjectDetail>,
  ...
  ngOnInit(): void {
    const {baremetalName, namespace, objectName} = this.activatedRoute_.snapshot.params;
    this.eventListEndpoint = this.endpoint_.child(
      `${baremetalName}/${objectName}`,
      Resource.event,
      namespace,
    );

    this.objectSubscription_ = this.object_
      .get(this.endpoint_.child(baremetalName, objectName, namespace))
      .subscribe((d: BaremetalObjectDetail) => {
        this.object = d;
